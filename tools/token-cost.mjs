...
function avcsRun(dir, source, sameLine) {
  ...
  try {
    ...
    if (sameLine) {
      ...
      const decided = sh("node", [join(ROOT, "tools", "resolve-conflict.mjs"), bob, "human:bob", "bob's change"], dir, env);
      if (!/decision recorded/.test(decided)) {
...
for (const size of SIZES) {
  ...
  for (const [scenario, sameLine] of [["disjoint edits", false], ["same-line collision", true]]) {
    const dir = mkdtempSync(join(tmpdir(), "avcs-tokencost-"));
    try {
      const git = gitRun(join(dir, "git"), source, sameLine) ?? null;
      const avcs = avcsRun(join(dir, "avcs"), source, sameLine);
...
console.log(pad("file", 16) + pad("scenario", 22) + lpad("git tok", 9) + lpad("avcs tok", 10) + lpad("saved", 8) + lpad("git trips", 11) + lpad("avcs trips", 12));
